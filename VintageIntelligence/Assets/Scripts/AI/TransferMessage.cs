...
public class TransferMessage : MonoBehaviourPun
{
    ...
    private void Start()
    {
        SpeechManager.Instance.OnNewRecognizedText += SendStringToOtherPlayers;  // Subscribe to the SpeechManager to receive the recorded text
    }
    ...
    public void SendStringToOtherPlayers(string message)
    {
        ...
        SpeechManager.Instance.StartReadMessage(message);
    }
    ...
    private void ReceiveString(string receivedString)
    {
        ...
        SpeechManager.Instance.StartReadMessage(receivedString); // Tells the speech manager to start converting to chatGPT and reading out the message.
